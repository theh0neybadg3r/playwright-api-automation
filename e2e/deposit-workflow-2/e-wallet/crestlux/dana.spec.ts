...
test.describe('CRESTLUX_DANA DEPOSIT WORKFLOW', () => {
    ...
    let cl_dana_Solution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        cl_dana_Solution = await DepositIntentRequest({ 
            solutionConfig: E_WALLET_SOLUTIONS.CL_Dana,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(cl_dana_Solution, 'DANA', VENDOR.CRESTLUX, SHEET_NAME.E_WALLET);
        await runNoErrorChecker(cl_dana_Solution, 'DANA', VENDOR.CRESTLUX, SHEET_NAME.E_WALLET);
        await runSuccessFlagChecker(cl_dana_Solution, 'DANA', VENDOR.CRESTLUX, SHEET_NAME.E_WALLET);
        await runCheckoutUrlChecker(cl_dana_Solution, 'DANA', VENDOR.CRESTLUX, SHEET_NAME.E_WALLET);

        //Checker for checking if the checkout page load without error.
        if (!cl_dana_Solution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(cl_dana_Solution.checkoutUrl, { waitUntil: 'load', timeout: 80000 });
            ...
            if (initialErrors.length > 0 || postInteractionErrors.length > 0) {
                ...
                await page.screenshot({ path: `error-logs/e-wallet/crestlux/dana/dana-error.png` });
            }
