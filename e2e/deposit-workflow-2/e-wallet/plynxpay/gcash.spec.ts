...
test.describe('GCASH DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let gcashSolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        gcashSolution = await DepositIntentRequest({ 
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(gcashSolution, 'Gcash', VENDOR.PLYNXPAY, SHEET_NAME.E_WALLET);
        await runNoErrorChecker(gcashSolution, 'Gcash', VENDOR.PLYNXPAY, SHEET_NAME.E_WALLET);
        await runSuccessFlagChecker(gcashSolution, 'Gcash', VENDOR.PLYNXPAY, SHEET_NAME.E_WALLET);
        await runCheckoutUrlChecker(gcashSolution, 'Gcash', VENDOR.PLYNXPAY, SHEET_NAME.E_WALLET);

        //Checker for checking if the checkout page load without error.
        if (!gcashSolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(gcashSolution.checkoutUrl, { waitUntil: 'networkidle', timeout: 80000 });
            ...
            if (initialErrors.length > 0 || postInteractionErrors.length > 0) {
                ...
                await page.screenshot({ path: `error-logs/e-wallet/plynxpay/gcash/gcash-error.png` });
            }
