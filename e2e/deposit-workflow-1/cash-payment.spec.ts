...
test.describe('CEBUANA DEPOSIT WORKFLOW', () => {
    ...
    test.beforeAll(async () => {
        cebuanaSolution = await DepositIntentRequest(CASH_PAYMENT_SOLUTIONS.Cebuana)
    });
    ...
    test.describe('API AND CHECKOUT PAGE VALIDATION', () => {
        ...
        test('should load checkout page without errors', async ({ page }) => {
            ...
            test.skip(!cebuanaSolution.checkoutUrl, 'No checkout URL available');
            ...
            await page.goto(cebuanaSolution.checkoutUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
            console.log('Redirected to:', page.url());
            await page.locator('body').waitFor({ state: 'visible', timeout: 10000 });
            ...
            const checkoutPageLoadResult = CHECKOUT_PAGE_CHECKER(foundErrorsInCheckoutPage, pageLoadTime, cebuanaSolution.checkoutUrl, 'Cebuana', VENDOR.UPAY, SHEET_NAME.CASH_PAYMENT);
