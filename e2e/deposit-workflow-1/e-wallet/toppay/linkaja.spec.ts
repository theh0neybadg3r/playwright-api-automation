...
test.describe('LINKAJA DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let linkAjaSolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        linkAjaSolution = await DepositIntentRequest({ 
            solutionConfig: E_WALLET_SOLUTIONS.LinkAja,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(linkAjaSolution, 'LinkAja', VENDOR.TOPPAY, SHEET_NAME.E_WALLET);
        await runNoErrorChecker(linkAjaSolution, 'LinkAja', VENDOR.TOPPAY, SHEET_NAME.E_WALLET);
        await runSuccessFlagChecker(linkAjaSolution, 'LinkAja', VENDOR.TOPPAY, SHEET_NAME.E_WALLET);
        await runCheckoutUrlChecker(linkAjaSolution, 'LinkAja', VENDOR.TOPPAY, SHEET_NAME.E_WALLET);

        //Checker for checking if the checkout page load without error.
        if (!linkAjaSolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(linkAjaSolution.checkoutUrl, { waitUntil: 'load', timeout: 80000 });
            console.log('Redirected to:', page.url());
            await page.locator('body').waitFor({ state: 'visible', timeout: 10000 });

            const { initialErrors, postInteractionErrors, interacted } = await paymentSelectionInteraction(page, ERROR_KEYWORDS, { label: 'LinkAja', aliases: ['linkaja'] });
            ...
            if (initialErrors.length > 0 || postInteractionErrors.length > 0) {
                ...
                await page.screenshot({ path: `error-logs/e-wallet/toppay/linkaja/linkaja-error.png` });
            }
