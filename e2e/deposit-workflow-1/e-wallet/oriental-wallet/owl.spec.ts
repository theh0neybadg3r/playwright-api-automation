...
test.describe('ORIENTAL_WALLET DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let owlSolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        owlSolution = await DepositIntentRequest({ 
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(owlSolution, 'Oriental Wallet', VENDOR.ORIENTAL_WALLET, SHEET_NAME.E_WALLET);
        await runNoErrorChecker(owlSolution, 'Oriental Wallet', VENDOR.ORIENTAL_WALLET, SHEET_NAME.E_WALLET);
        await runSuccessFlagChecker(owlSolution, 'Oriental Wallet', VENDOR.ORIENTAL_WALLET, SHEET_NAME.E_WALLET);
        await runCheckoutUrlChecker(owlSolution, 'Oriental Wallet', VENDOR.ORIENTAL_WALLET, SHEET_NAME.E_WALLET);

        //Checker for checking if the checkout page load without error.
        if (!owlSolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(owlSolution.checkoutUrl, { waitUntil: 'load', timeout: 80000 });
