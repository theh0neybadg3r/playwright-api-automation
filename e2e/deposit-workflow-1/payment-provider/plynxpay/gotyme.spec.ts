...
test.describe('GOTYME DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let gotymeSolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        gotymeSolution = await DepositIntentRequest({ 
            solutionConfig: PAYMENT_PROVIDER_SOLUTIONS.GoTyme,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(gotymeSolution, 'GoTyme', VENDOR.PLYNXPAY, SHEET_NAME.PAYMENT_PROVIDER);
        await runNoErrorChecker(gotymeSolution, 'GoTyme', VENDOR.PLYNXPAY, SHEET_NAME.PAYMENT_PROVIDER);
        await runSuccessFlagChecker(gotymeSolution, 'GoTyme', VENDOR.PLYNXPAY, SHEET_NAME.PAYMENT_PROVIDER);
        await runCheckoutUrlChecker(gotymeSolution, 'GoTyme', VENDOR.PLYNXPAY, SHEET_NAME.PAYMENT_PROVIDER);

        //Checker for checking if the checkout page load without error.
        if (!gotymeSolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(gotymeSolution.checkoutUrl, { waitUntil: 'networkidle', timeout: 80000 });
