...
test.describe('TRINITY_DUITNOW_QR DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let trinity_Duitnow_QR_Solution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        trinity_Duitnow_QR_Solution = await DepositIntentRequest({ 
            solutionConfig: PAYMENT_PROVIDER_SOLUTIONS.TRINITY_DUITNOW_QR,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(trinity_Duitnow_QR_Solution, 'Duitnow QR', VENDOR.TRINITY, SHEET_NAME.PAYMENT_PROVIDER);
        await runNoErrorChecker(trinity_Duitnow_QR_Solution, 'Duitnow QR', VENDOR.TRINITY, SHEET_NAME.PAYMENT_PROVIDER);
        await runSuccessFlagChecker(trinity_Duitnow_QR_Solution, 'Duitnow QR', VENDOR.TRINITY, SHEET_NAME.PAYMENT_PROVIDER);
        await runCheckoutUrlChecker(trinity_Duitnow_QR_Solution, 'Duitnow QR', VENDOR.TRINITY, SHEET_NAME.PAYMENT_PROVIDER);

        //Checker for checking if the checkout page load without error.
        if (!trinity_Duitnow_QR_Solution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(trinity_Duitnow_QR_Solution.checkoutUrl, { waitUntil: 'load', timeout: 80000 });
            ...
            const checkoutResult = CHECKOUT_INTERACTION_CHECKER(
                ...
                interacted, 
                pageLoadTime,
                trinity_Duitnow_QR_Solution.checkoutUrl,
                'Duitnow QR',
                VENDOR.TRINITY,
                SHEET_NAME.PAYMENT_PROVIDER
            );
