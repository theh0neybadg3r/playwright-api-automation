...
test.describe('QRPH DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let qrphSolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        qrphSolution = await DepositIntentRequest({ 
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(qrphSolution, 'QRPH', VENDOR.COINSPH, SHEET_NAME.PAYMENT_PROVIDER);
        await runNoErrorChecker(qrphSolution, 'QRPH', VENDOR.COINSPH, SHEET_NAME.PAYMENT_PROVIDER);
        await runSuccessFlagChecker(qrphSolution, 'QRPH', VENDOR.COINSPH, SHEET_NAME.PAYMENT_PROVIDER);
        await runCheckoutUrlChecker(qrphSolution, 'QRPH', VENDOR.COINSPH, SHEET_NAME.PAYMENT_PROVIDER);

        //Checker for checking if the checkout page load without error.
        if (!qrphSolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(qrphSolution.checkoutUrl, { waitUntil: 'networkidle', timeout: 80000 });
