...
test.describe('TOPPAY_QRPay DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let toppay_qrpay_Solution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        toppay_qrpay_Solution = await DepositIntentRequest({ 
            solutionConfig: PAYMENT_PROVIDER_SOLUTIONS.QRPay,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(toppay_qrpay_Solution, 'QRPay', VENDOR.TOPPAY, SHEET_NAME.PAYMENT_PROVIDER);
        await runNoErrorChecker(toppay_qrpay_Solution, 'QRPay', VENDOR.TOPPAY, SHEET_NAME.PAYMENT_PROVIDER);
        await runSuccessFlagChecker(toppay_qrpay_Solution, 'QRPay', VENDOR.TOPPAY, SHEET_NAME.PAYMENT_PROVIDER);
        await runCheckoutUrlChecker(toppay_qrpay_Solution, 'QRPay', VENDOR.TOPPAY, SHEET_NAME.PAYMENT_PROVIDER);

        //Checker for checking if the checkout page load without error.
        if (!toppay_qrpay_Solution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(toppay_qrpay_Solution.checkoutUrl, { waitUntil: 'load', timeout: 80000 });
            ...
            const checkoutResult = CHECKOUT_INTERACTION_CHECKER(
                ...
                interacted, 
                pageLoadTime,
                toppay_qrpay_Solution.checkoutUrl,
                'QRPay',
                VENDOR.TOPPAY,
                SHEET_NAME.PAYMENT_PROVIDER
            );
