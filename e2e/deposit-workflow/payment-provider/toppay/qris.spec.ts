...
test.describe('TOPPAY_QRIS DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let toppay_qris_Solution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        toppay_qris_Solution = await DepositIntentRequest({ 
            solutionConfig: PAYMENT_PROVIDER_SOLUTIONS.QRIS,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(toppay_qris_Solution, 'QRIS', VENDOR.TOPPAY, SHEET_NAME.PAYMENT_PROVIDER);
        await runNoErrorChecker(toppay_qris_Solution, 'QRIS', VENDOR.TOPPAY, SHEET_NAME.PAYMENT_PROVIDER);
        await runSuccessFlagChecker(toppay_qris_Solution, 'QRIS', VENDOR.TOPPAY, SHEET_NAME.PAYMENT_PROVIDER);
        await runCheckoutUrlChecker(toppay_qris_Solution, 'QRIS', VENDOR.TOPPAY, SHEET_NAME.PAYMENT_PROVIDER);

        //Checker for checking if the checkout page load without error.
        if (!toppay_qris_Solution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(toppay_qris_Solution.checkoutUrl, { waitUntil: 'load', timeout: 80000 });
            ...
            const checkoutResult = CHECKOUT_INTERACTION_CHECKER(
                ...
                interacted, 
                pageLoadTime,
                toppay_qris_Solution.checkoutUrl,
                'QRIS',
                VENDOR.TOPPAY,
                SHEET_NAME.PAYMENT_PROVIDER
            );
