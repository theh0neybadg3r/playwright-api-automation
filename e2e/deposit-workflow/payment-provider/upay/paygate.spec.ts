...
test.describe('PAYGATE DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let paygateSolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        paygateSolution = await DepositIntentRequest({ 
            solutionConfig: PAYMENT_PROVIDER_SOLUTIONS.Paygate,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(paygateSolution, 'Paygate', VENDOR.UPAY, SHEET_NAME.PAYMENT_PROVIDER);
        await runNoErrorChecker(paygateSolution, 'Paygate', VENDOR.UPAY, SHEET_NAME.PAYMENT_PROVIDER);
        await runSuccessFlagChecker(paygateSolution, 'Paygate', VENDOR.UPAY, SHEET_NAME.PAYMENT_PROVIDER);
        await runCheckoutUrlChecker(paygateSolution, 'Paygate', VENDOR.UPAY, SHEET_NAME.PAYMENT_PROVIDER);

        //Checker for checking if the checkout page load without error.
        if (!paygateSolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(paygateSolution.checkoutUrl, { waitUntil: 'networkidle', timeout: 80000 });
            console.log('Redirected to:', page.url());
            await page.locator('body').waitFor({ state: 'visible', timeout: 10000 });

            const { initialErrors, postInteractionErrors, interacted } = await checkoutInteraction (
                page, 
                ERROR_KEYWORDS,
                { 
                    loginOptionText: 'Log in to Online Banking', 
