...
test.describe('INSTAPAY DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let instapaySolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        instapaySolution = await DepositIntentRequest({ 
            solutionConfig: PAYMENT_PROVIDER_SOLUTIONS.Instapay,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(instapaySolution, 'Instapay', VENDOR.UPAY, SHEET_NAME.PAYMENT_PROVIDER);
        await runNoErrorChecker(instapaySolution, 'Instapay', VENDOR.UPAY, SHEET_NAME.PAYMENT_PROVIDER);
        await runSuccessFlagChecker(instapaySolution, 'Instapay', VENDOR.UPAY, SHEET_NAME.PAYMENT_PROVIDER);
        await runCheckoutUrlChecker(instapaySolution, 'Instapay', VENDOR.UPAY, SHEET_NAME.PAYMENT_PROVIDER);

        //Checker for checking if the checkout page load without error.
        if (!instapaySolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(instapaySolution.checkoutUrl, { waitUntil: 'networkidle', timeout: 80000 });
