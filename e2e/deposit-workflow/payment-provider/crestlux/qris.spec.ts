...
test.describe('CRESTLUX_QRIS DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let cl_qris_Solution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        cl_qris_Solution = await DepositIntentRequest({ 
            solutionConfig: PAYMENT_PROVIDER_SOLUTIONS.CL_QRIS,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(cl_qris_Solution, 'QRIS', VENDOR.CRESTLUX, SHEET_NAME.PAYMENT_PROVIDER);
        await runNoErrorChecker(cl_qris_Solution, 'QRIS', VENDOR.CRESTLUX, SHEET_NAME.PAYMENT_PROVIDER);
        await runSuccessFlagChecker(cl_qris_Solution, 'QRIS', VENDOR.CRESTLUX, SHEET_NAME.PAYMENT_PROVIDER);
        await runCheckoutUrlChecker(cl_qris_Solution, 'QRIS', VENDOR.CRESTLUX, SHEET_NAME.PAYMENT_PROVIDER);

        //Checker for checking if the checkout page load without error.
        if (!cl_qris_Solution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(cl_qris_Solution.checkoutUrl, { waitUntil: 'load', timeout: 80000 });
