...
test.describe('ALLBANK_QRPH DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let allBank_qrph_Solution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        allBank_qrph_Solution = await DepositIntentRequest({ 
            solutionConfig: PAYMENT_PROVIDER_SOLUTIONS.QRPH,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(allBank_qrph_Solution, 'QRPH', VENDOR.ALLBANK, SHEET_NAME.PAYMENT_PROVIDER);
        await runNoErrorChecker(allBank_qrph_Solution, 'QRPH', VENDOR.ALLBANK, SHEET_NAME.PAYMENT_PROVIDER);
        await runSuccessFlagChecker(allBank_qrph_Solution, 'QRPH', VENDOR.ALLBANK, SHEET_NAME.PAYMENT_PROVIDER);
        await runCheckoutUrlChecker(allBank_qrph_Solution, 'QRPH', VENDOR.ALLBANK, SHEET_NAME.PAYMENT_PROVIDER);

        //Checker for checking if the checkout page load without error.
        if (!allBank_qrph_Solution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(allBank_qrph_Solution.checkoutUrl, { waitUntil: 'networkidle', timeout: 80000 });
            ...
            const checkoutResult = CHECKOUT_INTERACTION_CHECKER(
                ...
                interacted, 
                pageLoadTime,
                allBank_qrph_Solution.checkoutUrl,
                'QRPH',
                VENDOR.ALLBANK,
                SHEET_NAME.PAYMENT_PROVIDER
            );
