...
test.describe('REBANQXPAY_LBT_AUSTRALIA DEPOSIT WORKFLOW', () => {
    ...
    let lbtAustraliaSolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        lbtAustraliaSolution = await DepositIntentRequest({ 
            solutionConfig: LOCAL_BANK_TRANSFER_SOLUTIONS.Local_Bank_Australia,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(lbtAustraliaSolution, 'Local Bank Australia', VENDOR.REBANQXPAY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runNoErrorChecker(lbtAustraliaSolution, 'Local Bank Australia', VENDOR.REBANQXPAY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runSuccessFlagChecker(lbtAustraliaSolution, 'Local Bank Australia', VENDOR.REBANQXPAY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runCheckoutUrlChecker(lbtAustraliaSolution, 'Local Bank Australia', VENDOR.REBANQXPAY, SHEET_NAME.LOCAL_BANK_TRANSFER);

        //Checker for checking if the checkout page load without error.
        if (!lbtAustraliaSolution?.checkoutUrl) {
            const failedResult = CHECKOUT_PAGE_CHECKER(
                ['No checkout URL available'], 
                0,
                null,
                'Local Bank Australia',
                VENDOR.REBANQXPAY,
        ...
        try {
            ...
            await page.goto(lbtAustraliaSolution.checkoutUrl, { waitUntil: 'load', timeout: 80000 });
            ...
            const checkoutResult = CHECKOUT_INTERACTION_CHECKER(
                ...
                interacted, 
                pageLoadTime,
                lbtAustraliaSolution.checkoutUrl,
                'Local Bank Australia',
                VENDOR.REBANQXPAY,
                SHEET_NAME.LOCAL_BANK_TRANSFER
            );
