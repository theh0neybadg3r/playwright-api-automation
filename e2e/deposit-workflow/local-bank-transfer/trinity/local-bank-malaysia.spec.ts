...
test.describe('TRINITY_LBT_MALAYSIA DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let trinity_lbtMalaysiaSolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        trinity_lbtMalaysiaSolution = await DepositIntentRequest({ 
            solutionConfig: LOCAL_BANK_TRANSFER_SOLUTIONS.TR_Local_Bank_Malaysia,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(trinity_lbtMalaysiaSolution, 'Local Bank Malaysia', VENDOR.TRINITY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runNoErrorChecker(trinity_lbtMalaysiaSolution, 'Local Bank Malaysia', VENDOR.TRINITY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runSuccessFlagChecker(trinity_lbtMalaysiaSolution, 'Local Bank Malaysia', VENDOR.TRINITY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runCheckoutUrlChecker(trinity_lbtMalaysiaSolution, 'Local Bank Malaysia', VENDOR.TRINITY, SHEET_NAME.LOCAL_BANK_TRANSFER);

        //Checker for checking if the checkout page load without error.
        if (!trinity_lbtMalaysiaSolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(trinity_lbtMalaysiaSolution.checkoutUrl, { waitUntil: 'load', timeout: 80000 });
            ...
            const checkoutResult = CHECKOUT_INTERACTION_CHECKER(
                ...
                interacted, 
                pageLoadTime,
                trinity_lbtMalaysiaSolution.checkoutUrl,
                'Local Bank Malaysia',
                VENDOR.TRINITY,
                SHEET_NAME.LOCAL_BANK_TRANSFER
            );
