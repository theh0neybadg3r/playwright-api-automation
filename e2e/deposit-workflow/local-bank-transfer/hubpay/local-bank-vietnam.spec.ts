...
test.describe('HUBPAY_LBT_VIETNAM DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let hubpay_lbtVietnamSolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        hubpay_lbtVietnamSolution = await DepositIntentRequest({ 
            solutionConfig: LOCAL_BANK_TRANSFER_SOLUTIONS.HP_Local_Bank_Vietnam,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(hubpay_lbtVietnamSolution, 'Local Bank Vietnam', VENDOR.HUBPAY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runNoErrorChecker(hubpay_lbtVietnamSolution, 'Local Bank Vietnam', VENDOR.HUBPAY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runSuccessFlagChecker(hubpay_lbtVietnamSolution, 'Local Bank Vietnam', VENDOR.HUBPAY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runCheckoutUrlChecker(hubpay_lbtVietnamSolution, 'Local Bank Vietnam', VENDOR.HUBPAY, SHEET_NAME.LOCAL_BANK_TRANSFER);

        //Checker for checking if the checkout page load without error.
        if (!hubpay_lbtVietnamSolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(hubpay_lbtVietnamSolution.checkoutUrl, { waitUntil: 'load', timeout: 80000 });
            ...
            const checkoutResult = CHECKOUT_INTERACTION_CHECKER(
                ...
                interacted, 
                pageLoadTime,
                hubpay_lbtVietnamSolution.checkoutUrl,
                'Local Bank Vietnam',
                VENDOR.HUBPAY,
                SHEET_NAME.LOCAL_BANK_TRANSFER
            );
