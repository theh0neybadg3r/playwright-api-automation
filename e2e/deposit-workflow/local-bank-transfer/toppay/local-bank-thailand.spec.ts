...
test.describe('TOPPAY_LBT_THAILAND DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let toppay_lbtThailandSolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        toppay_lbtThailandSolution = await DepositIntentRequest({ 
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(toppay_lbtThailandSolution, 'Local Bank Thailand', VENDOR.TOPPAY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runNoErrorChecker(toppay_lbtThailandSolution, 'Local Bank Thailand', VENDOR.TOPPAY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runSuccessFlagChecker(toppay_lbtThailandSolution, 'Local Bank Thailand', VENDOR.TOPPAY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runCheckoutUrlChecker(toppay_lbtThailandSolution, 'Local Bank Thailand', VENDOR.TOPPAY, SHEET_NAME.LOCAL_BANK_TRANSFER);

        //Checker for checking if the checkout page load without error.
        if (!toppay_lbtThailandSolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(toppay_lbtThailandSolution.checkoutUrl, { waitUntil: 'load', timeout: 80000 });
            ...
            const checkoutResult = CHECKOUT_INTERACTION_CHECKER(
                ...
                interacted, 
                pageLoadTime,
                toppay_lbtThailandSolution.checkoutUrl,
                'Local Bank Thailand',
                VENDOR.TOPPAY,
                SHEET_NAME.LOCAL_BANK_TRANSFER
            );
