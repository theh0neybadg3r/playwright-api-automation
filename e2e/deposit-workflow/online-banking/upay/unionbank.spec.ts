...
test.describe('UNIONBANK DEPOSIT WORKFLOW', () => {
    ...
    test.beforeAll(async () => {
        ...
        unionbankSolution = await DepositIntentRequest({ 
            solutionConfig: ONLINE_BANKING_SOLUTION.Unionbank,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        if (!unionbankSolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(unionbankSolution.checkoutUrl, { waitUntil: 'networkidle', timeout: 80000 });
            ...
            if (initialErrors.length > 0 || postInteractionErrors.length > 0) {
                ...
                await page.screenshot({ path: `error-logs/online-banking/upay/unionbank/unionbank-error.png` });
            }
