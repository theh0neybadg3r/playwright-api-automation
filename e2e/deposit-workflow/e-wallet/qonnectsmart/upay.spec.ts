...
test.describe('NAGAD DEPOSIT WORKFLOW', () => {
    ...
    let upaySolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        upaySolution = await DepositIntentRequest({ 
            solutionConfig: E_WALLET_SOLUTIONS.Upay,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(upaySolution, 'Upay', VENDOR.QONNECTSMART, SHEET_NAME.E_WALLET);
        await runNoErrorChecker(upaySolution, 'Upay', VENDOR.QONNECTSMART, SHEET_NAME.E_WALLET);
        await runSuccessFlagChecker(upaySolution, 'Upay', VENDOR.QONNECTSMART, SHEET_NAME.E_WALLET);
        await runCheckoutUrlChecker(upaySolution, 'Upay', VENDOR.QONNECTSMART, SHEET_NAME.E_WALLET);

        //Checker for checking if the checkout page load without error.
        if (!upaySolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(upaySolution.checkoutUrl, { waitUntil: 'networkidle', timeout: 80000 });
            ...
            if (initialErrors.length > 0 || postInteractionErrors.length > 0) {
                ...
                await page.screenshot({ path: `error-logs/e-wallet/qonnectsmart/upay/upay-error.png` });
            }
