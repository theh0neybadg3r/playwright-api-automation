...
test.describe('ROCKET DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let rocketSolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        rocketSolution = await DepositIntentRequest({ 
            solutionConfig: E_WALLET_SOLUTIONS.Rocket,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(rocketSolution, 'Rocket', VENDOR.QONNECTSMART, SHEET_NAME.E_WALLET);
        await runNoErrorChecker(rocketSolution, 'Rocket', VENDOR.QONNECTSMART, SHEET_NAME.E_WALLET);
        await runSuccessFlagChecker(rocketSolution, 'Rocket', VENDOR.QONNECTSMART, SHEET_NAME.E_WALLET);
        await runCheckoutUrlChecker(rocketSolution, 'Rocket', VENDOR.QONNECTSMART, SHEET_NAME.E_WALLET);

        //Checker for checking if the checkout page load without error.
        if (!rocketSolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(rocketSolution.checkoutUrl, { waitUntil: 'networkidle', timeout: 80000 });
            ...
            if (initialErrors.length > 0 || postInteractionErrors.length > 0) {
                ...
                await page.screenshot({ path: `error-logs/e-wallet/qonnectsmart/rocket/rocket-error.png` });
            }
