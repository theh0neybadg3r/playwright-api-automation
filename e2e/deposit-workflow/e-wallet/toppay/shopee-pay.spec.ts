...
test.describe('SHOPEE PAY DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let sPaySolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        sPaySolution = await DepositIntentRequest({ 
            solutionConfig: E_WALLET_SOLUTIONS.ShopeePay,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(sPaySolution, 'ShopeePay', VENDOR.TOPPAY, SHEET_NAME.E_WALLET);
        await runNoErrorChecker(sPaySolution, 'ShopeePay', VENDOR.TOPPAY, SHEET_NAME.E_WALLET);
        await runSuccessFlagChecker(sPaySolution, 'ShopeePay', VENDOR.TOPPAY, SHEET_NAME.E_WALLET);
        await runCheckoutUrlChecker(sPaySolution, 'ShopeePay', VENDOR.TOPPAY, SHEET_NAME.E_WALLET);

        //Checker for checking if the checkout page load without error.
        if (!sPaySolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(sPaySolution.checkoutUrl, { waitUntil: 'load', timeout: 80000 });
            console.log('Redirected to:', page.url());
            await page.locator('body').waitFor({ state: 'visible', timeout: 10000 });

            const { initialErrors, postInteractionErrors, interacted } = await paymentSelectionInteraction(page, ERROR_KEYWORDS, { label: 'ShopeePay', aliases: ['shopeepay'] });
