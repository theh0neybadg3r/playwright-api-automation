...
test.describe('ZALO DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let zaloSolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        zaloSolution = await DepositIntentRequest({ 
            solutionConfig: E_WALLET_SOLUTIONS.Zalo,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(zaloSolution, 'Zalo', VENDOR.DELTA_ASET, SHEET_NAME.E_WALLET);
        await runNoErrorChecker(zaloSolution, 'Zalo', VENDOR.DELTA_ASET, SHEET_NAME.E_WALLET);
        await runSuccessFlagChecker(zaloSolution, 'Zalo', VENDOR.DELTA_ASET, SHEET_NAME.E_WALLET);
        await runCheckoutUrlChecker(zaloSolution, 'Zalo', VENDOR.DELTA_ASET, SHEET_NAME.E_WALLET);

        //Checker for checking if the checkout page load without error.
        if (!zaloSolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(zaloSolution.checkoutUrl, { waitUntil: 'load', timeout: 80000 });
            ...
            if (initialErrors.length > 0 || postInteractionErrors.length > 0) {
                ...
                await page.screenshot({ path: `error-logs/e-wallet/delta-aset/zalo/zalo-error.png` });
            }
