...
test.describe('VIETTELPAY DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let viettelpaySolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        viettelpaySolution = await DepositIntentRequest({ 
            solutionConfig: E_WALLET_SOLUTIONS.ViettelPay,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(viettelpaySolution, 'ViettelPay', VENDOR.DELTA_ASET, SHEET_NAME.E_WALLET);
        await runNoErrorChecker(viettelpaySolution, 'ViettelPay', VENDOR.DELTA_ASET, SHEET_NAME.E_WALLET);
        await runSuccessFlagChecker(viettelpaySolution, 'ViettelPay', VENDOR.DELTA_ASET, SHEET_NAME.E_WALLET);
        await runCheckoutUrlChecker(viettelpaySolution, 'ViettelPay', VENDOR.DELTA_ASET, SHEET_NAME.E_WALLET);

        //Checker for checking if the checkout page load without error.
        if (!viettelpaySolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(viettelpaySolution.checkoutUrl, { waitUntil: 'load', timeout: 80000 });
            ...
            if (initialErrors.length > 0 || postInteractionErrors.length > 0) {
                ...
                await page.screenshot({ path: `error-logs/e-wallet/delta-aset/viettel-pay/viettelpay-error.png` });
            }
            ...
            const checkoutResult = CHECKOUT_INTERACTION_CHECKER(
                ...
                interacted, 
                pageLoadTime,
                viettelpaySolution.checkoutUrl,
                'ViettelPay',
                VENDOR.DELTA_ASET,
                SHEET_NAME.E_WALLET
            );
