...
test.describe('AMB_PAY_SPay_MYR DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let amb_pay_SPay_MYR_Solution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        amb_pay_SPay_MYR_Solution = await DepositIntentRequest({ 
            solutionConfig: E_WALLET_SOLUTIONS.AMB_Pay_Shopeepay_MYR,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(amb_pay_SPay_MYR_Solution, 'Shopee Pay (MYR)', VENDOR.AMB_PAY, SHEET_NAME.E_WALLET);
        await runNoErrorChecker(amb_pay_SPay_MYR_Solution, 'Shopee Pay (MYR)', VENDOR.AMB_PAY, SHEET_NAME.E_WALLET);
        await runSuccessFlagChecker(amb_pay_SPay_MYR_Solution, 'Shopee Pay (MYR)', VENDOR.AMB_PAY, SHEET_NAME.E_WALLET);
        await runCheckoutUrlChecker(amb_pay_SPay_MYR_Solution, 'Shopee Pay (MYR)', VENDOR.AMB_PAY, SHEET_NAME.E_WALLET);

        //Checker for checking if the checkout page load without error.
        if (!amb_pay_SPay_MYR_Solution?.checkoutUrl) {
        ...
        try {
            ...
            const checkoutResult = CHECKOUT_INTERACTION_CHECKER(
                ...
                interacted, 
                pageLoadTime,
                amb_pay_SPay_MYR_Solution.checkoutUrl,
                'Shopee Pay (MYR)',
                VENDOR.AMB_PAY,
                SHEET_NAME.E_WALLET
            );
