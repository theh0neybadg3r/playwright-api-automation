...
test.describe('AMB_PAY_DANA DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let amb_pay_Dana_Solution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        amb_pay_Dana_Solution = await DepositIntentRequest({ 
            solutionConfig: E_WALLET_SOLUTIONS.AMB_Pay_Dana,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(amb_pay_Dana_Solution, 'DANA', VENDOR.AMB_PAY, SHEET_NAME.E_WALLET);
        await runNoErrorChecker(amb_pay_Dana_Solution, 'DANA', VENDOR.AMB_PAY, SHEET_NAME.E_WALLET);
        await runSuccessFlagChecker(amb_pay_Dana_Solution, 'DANA', VENDOR.AMB_PAY, SHEET_NAME.E_WALLET);
        await runCheckoutUrlChecker(amb_pay_Dana_Solution, 'DANA', VENDOR.AMB_PAY, SHEET_NAME.E_WALLET);

        //Checker for checking if the checkout page load without error.
        if (!amb_pay_Dana_Solution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(amb_pay_Dana_Solution.checkoutUrl, { waitUntil: 'load', timeout: 80000 });
            ...
            const checkoutResult = CHECKOUT_INTERACTION_CHECKER(
                ...
                interacted, 
                pageLoadTime,
                amb_pay_Dana_Solution.checkoutUrl,
                'DANA',
                VENDOR.AMB_PAY,
                SHEET_NAME.E_WALLET
            );
