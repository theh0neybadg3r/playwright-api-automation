...
test.describe('TRINITY_LBT_KOREA DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let trinity_lbtKoreaSolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        trinity_lbtKoreaSolution = await DepositIntentRequest({ 
            solutionConfig: LOCAL_BANK_TRANSFER_SOLUTIONS.TR_Local_Bank_Korea,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(trinity_lbtKoreaSolution, 'Local Bank Korea', VENDOR.TRINITY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runNoErrorChecker(trinity_lbtKoreaSolution, 'Local Bank Korea', VENDOR.TRINITY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runSuccessFlagChecker(trinity_lbtKoreaSolution, 'Local Bank Korea', VENDOR.TRINITY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runCheckoutUrlChecker(trinity_lbtKoreaSolution, 'Local Bank Korea', VENDOR.TRINITY, SHEET_NAME.LOCAL_BANK_TRANSFER);

        //Checker for checking if the checkout page load without error.
        if (!trinity_lbtKoreaSolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(trinity_lbtKoreaSolution.checkoutUrl, { waitUntil: 'load', timeout: 80000 });
            ...
            const checkoutResult = CHECKOUT_INTERACTION_CHECKER(
                ...
                interacted, 
                pageLoadTime,
                trinity_lbtKoreaSolution.checkoutUrl,
                'Local Bank Korea',
                VENDOR.TRINITY,
                SHEET_NAME.LOCAL_BANK_TRANSFER
            );
