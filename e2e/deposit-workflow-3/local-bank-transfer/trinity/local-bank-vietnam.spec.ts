...
test.describe('TRINITY_LBT_VIETNAM DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let trinity_lbtVietnamSolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        trinity_lbtVietnamSolution = await DepositIntentRequest({ 
            solutionConfig: LOCAL_BANK_TRANSFER_SOLUTIONS.TR_Local_Bank_Vietnam,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(trinity_lbtVietnamSolution, 'Local Bank Vietnam', VENDOR.TRINITY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runNoErrorChecker(trinity_lbtVietnamSolution, 'Local Bank Vietnam', VENDOR.TRINITY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runSuccessFlagChecker(trinity_lbtVietnamSolution, 'Local Bank Vietnam', VENDOR.TRINITY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runCheckoutUrlChecker(trinity_lbtVietnamSolution, 'Local Bank Vietnam', VENDOR.TRINITY, SHEET_NAME.LOCAL_BANK_TRANSFER);

        //Checker for checking if the checkout page load without error.
        if (!trinity_lbtVietnamSolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(trinity_lbtVietnamSolution.checkoutUrl, { waitUntil: 'load', timeout: 80000 });
            ...
            const checkoutResult = CHECKOUT_INTERACTION_CHECKER(
                ...
                interacted, 
                pageLoadTime,
                trinity_lbtVietnamSolution.checkoutUrl,
                'Local Bank Vietnam',
                VENDOR.TRINITY,
                SHEET_NAME.LOCAL_BANK_TRANSFER
            );
