...
test.describe('TRINITY_LBT_THAILAND DEPOSIT WORKFLOW', () => {

    test.describe.configure({ mode: 'serial' });

    let trinity_lbtThailandSolution: DepositInterface;

    test.beforeAll(async () => {

        // test.setTimeout(120000);

        trinity_lbtThailandSolution = await DepositIntentRequest({ 
            solutionConfig: LOCAL_BANK_TRANSFER_SOLUTIONS.TP_Local_Bank_Thailand,
            apiKeys: {
    ...
    test('API AND CHECKOUT PAGE VALIDATION', async ({ browser }) => {
        ...
        await runStatusCodeChecker(trinity_lbtThailandSolution, 'Local Bank Thailand', VENDOR.TRINITY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runNoErrorChecker(trinity_lbtThailandSolution, 'Local Bank Thailand', VENDOR.TRINITY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runSuccessFlagChecker(trinity_lbtThailandSolution, 'Local Bank Thailand', VENDOR.TRINITY, SHEET_NAME.LOCAL_BANK_TRANSFER);
        await runCheckoutUrlChecker(trinity_lbtThailandSolution, 'Local Bank Thailand', VENDOR.TRINITY, SHEET_NAME.LOCAL_BANK_TRANSFER);

        //Checker for checking if the checkout page load without error.
        if (!trinity_lbtThailandSolution?.checkoutUrl) {
        ...
        try {
            ...
            await page.goto(trinity_lbtThailandSolution.checkoutUrl, { waitUntil: 'load', timeout: 80000 });
            ...
            const checkoutResult = CHECKOUT_INTERACTION_CHECKER(
                ...
                interacted, 
                pageLoadTime,
                trinity_lbtThailandSolution.checkoutUrl,
                'Local Bank Thailand',
                VENDOR.TRINITY,
                SHEET_NAME.LOCAL_BANK_TRANSFER
            );
